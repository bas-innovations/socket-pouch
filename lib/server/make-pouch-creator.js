...
function createHttpPouch(options) {
  ...
  return function (args) {
    ...
    console.log('Before insertion; remoteUrl: ',remoteUrl);
    if (args[0].authorization) {
      ...
      var protocol = 'http://'
      if (doubleSlashIdx !== -1) {
        protocol = remoteUrl.substring(0, doubleSlashIdx + 2);
        remoteUrl = remoteUrl.substring(doubleSlashIdx + 2);
      }
      // insert Authorisation to url
      remoteUrl = protocol + token + ':' + password + '@' + remoteUrl;
      console.log('After insertion; remoteUrl: ',remoteUrl);
    } else {
